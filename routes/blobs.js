...
router.route('/')
    ...
    .post(function(req, res) {
        ...
        var clanovi_projekta =req.body.clanovi_projekta;
        ...
        mongoose.model('Blob').create({
            ...
            clanovi_projekta : clanovi_projekta
        }, function (err, blob) {
              if (err) {
...
router.route('/:id/edit')
	...
	.get(function(req, res) {
	    ...
	    mongoose.model('Blob').findById(req.id, function (err, blob) {
	        if (err) {
	            ...
	            //Return the blob
                console.log('GET Retrieving ID: ' + blob._id);
                var blobdatum_pocetka = blob.datum_pocetka.toISOString();
                blobdatum_pocetka = blobdatum_pocetka.substring(0, blobdatum_pocetka.indexOf('T'));

                var blobdatum_zavrsetka = blob.datum_zavrsetka.toISOString();
                blobdatum_zavrsetka = blobdatum_zavrsetka.substring(0, blobdatum_zavrsetka.indexOf('T'));
	            res.format({
	                ...
	                html: function(){
	                       res.render('blobs/edit', {
	                          title: 'Blob' + blob._id,
                              "blob" : blob,
                              "datumpocetka" : blobdatum_pocetka,
                              "datumzavrsetka": blobdatum_zavrsetka
	                      });
	                 },
	...
	.put(function(req, res) {
	    // Get our REST or form values. These rely on the "naziv_projekta" attributes
	    var naziv_projekta = req.body.naziv_projekta;
        var opis_projekta = req.body.opis_projekta;
        var cijena_projekta = req.body.cijena_projekta;
        var datum_pocetka = req.body.datum_pocetka;
